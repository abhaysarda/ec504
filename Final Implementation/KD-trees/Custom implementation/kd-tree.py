...
class KDTree(object):
    ...
    def nearest_neighbor(self, destination):
        ...
        def recursive_search(here):
            ...
            here_sd = euclidean(point, destination)
            if here_sd < best[2]:
...
if __name__ == '__main__':
    ...
    points = [(tuple(random() for _ in range(k)), i)
    ...
    for _ in range(lookups):
        ...
        destination = [random() for _ in range(k)]
        _, _, mindistance = tree.nearest_neighbor(destination)

        minsq = min(euclidean(p, destination) for p, _ in points)
